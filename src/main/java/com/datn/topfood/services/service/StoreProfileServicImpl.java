...
public class StoreProfileServicImpl extends BaseService implements StoreProfileServic {
    ...
    public StoreWallResponse storeProfile(Long storeProfileId) {
        ...
        swr.setFoods(foodRepository.findByProfileId(storeProfileId).stream().map((food) ->
                new FoodDetailResponse(food.getId(), food.getName(), food.getPrice(), food.getContent(),
    ...
    public PageResponse<FoodDetailResponse> listFood(PageRequest pageRequest) {
        ...
        return new PageResponse<>(foods.stream().map((food) ->
                new FoodDetailResponse(food.getId(), food.getName(), food.getPrice(), food.getContent(),
                        food.getFiles().stream().map((file) -> file.getPath()).collect(Collectors.toList()),
                        new TagResponse(food.getTag().getId(), food.getTag().getTagName()))
    ...
    public FoodDetailResponse updateFood(FoodRequest foodRequest) {
        ...
        return new FoodDetailResponse(food.getId(), food.getName(), food.getPrice(), food.getContent(),
                food.getFiles().stream().map((file) -> file.getPath()).collect(Collectors.toList()), new TagResponse(food.getTag().getId(), food.getTag().getTagName()));
    }
    ...
    public PostResponse createPost(PostRequest postRequest) {
        ...
        return new PostResponse(p.getId(), p.getContent(), ConvertUtils.convertSetToArrFile(p.getFiles()),
                p.getTags().stream().map((tag) -> new TagResponse(tag.getId(), tag.getTagName())
                ).collect(Collectors.toList()));
    }
    ...
    public PostResponse detailPost(Long id) {
        ...
        assert p != null;
        return new PostResponse(p.getId(), p.getContent(), ConvertUtils.convertSetToArrFile(p.getFiles()), p.getTags().stream().map((t) -> new TagResponse(t.getId(), t.getTagName())).collect(Collectors.toList()));
    }
    ...
    public PageResponse<PostResponse> getListPost(PageRequest pageRequest) {
        ...
        listPost.forEach((p) ->
                listPostResponse.add(
                        new PostResponse(p.getId(), p.getContent(),
                                p.getFiles().stream().map((f) -> f.getPath()).collect(Collectors.toList()),
                                p.getTags().stream().map((t) -> new TagResponse(t.getId(), t.getTagName())).collect(Collectors.toList())
                        )
                )
        );
    ...
    public PostResponse updatePost(PostRequest postRequest) {
        ...
        postResponse.setTags(post.getTags().stream().map((tag) -> new TagResponse(tag.getId(), tag.getTagName())).collect(Collectors.toList()));
    ...
    public PageResponse<FoodDetailResponse> searchFoods(SearchFoodsRequest foodsRequest, PageRequest pageRequest) {
        ...
        return new PageResponse<>(response, storeCustomRepository.countFoodsSearch(foodsRequest.getFoodName(),
